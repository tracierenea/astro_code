function [x_hat, fem_states] = nonlinear_least_squares_orbit_det(time, ...
    x0_guess, y_meas, Phi0_vec, invR, max_count, mom_states, freq, mu)
...
while (count <= max_count) && (cost_change > 0.01)
  ...
  [~, results]     = ode45(@NLS_sys_eqns, time, big_vector, [], mu, n);
  fem_states       = results(:,1:n);
  ...
  % Update the estimate
  x_hat            = x_hat + del_x;
  % Increment the counter and cost-tracking variable
  count            = count + 1;
  cost_change      = abs((cost - last_cost)/last_cost);
  last_cost        = cost;
end